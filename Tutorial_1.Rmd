...
# Before we can pull packages into 'R', you will first have to install them onto your computer. Run the following code to download the required packages:
...
nestling <- read.csv("TRES/nestling_final.csv")
```
...
head(banding) #where head() is the function and banding is the variable
```
...
#Say you want to create a new column in the banding2 dataset where you computer the mean weight of all the birds
...
banding3 <- aggregate(weight ~ sex, banding, mean) # bandingF is where these values will be stored
banding3
```
...
# Remember, if you want to store this in a df to look at later I would have to assign it to a *vector* (We are going to call it "female_birds")
...
#Notice how the vector female_birds changed from 992 observations to 567 observations?
...
Congrats! You've made your first graph. We can change elements of the graph by adding lables and titles, changing the theme and colours of our bars. 
...
What trends do you see? Why do you think there are more recaptured birds compared to banded birds every year?
...
Next, let's take a quick look at the egg nestling dataset
...
We see the weight is a character vector. Let's change that to numeric using the as.numeric() function. Let's start with the **day_1_weight** column:
